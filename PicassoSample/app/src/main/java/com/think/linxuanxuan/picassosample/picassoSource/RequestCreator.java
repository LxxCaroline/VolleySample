...
import static com.think.linxuanxuan.picassosample.picassoSource.BitmapHunter.forRequest;
import static com.think.linxuanxuan.picassosample.picassoSource.MemoryPolicy.NO_CACHE;
import static com.think.linxuanxuan.picassosample.picassoSource.MemoryPolicy.NO_STORE;
import static com.think.linxuanxuan.picassosample.picassoSource.MemoryPolicy.shouldReadFromMemoryCache;
import static com.think.linxuanxuan.picassosample.picassoSource.PicassoDrawable.setBitmap;
import static com.think.linxuanxuan.picassosample.picassoSource.PicassoDrawable.setPlaceholder;
...
public class RequestCreator {
   ...
   * {@link #fetch()} requests, which have {@link Picasso.Priority#LOW} priority by default.
   */
      ...
      if (shouldReadFromMemoryCache(memoryPolicy)) {
        ...
        if (bitmap != null) {
          if (picasso.loggingEnabled) {
            Utils.log(Utils.OWNER_MAIN, Utils.VERB_COMPLETED, request.plainId(), "from " + Picasso.LoadedFrom.MEMORY);
          }
          if (callback != null) {
            callback.onSuccess();
          }
  ...
  public void into(ImageView target, Callback callback) {
    ...
    if (shouldReadFromMemoryCache(memoryPolicy)) {
      ...
      if (bitmap != null) {
        picasso.cancelRequest(target);
        setBitmap(target, picasso.context, bitmap, Picasso.LoadedFrom.MEMORY, noFade, picasso.indicatorsEnabled);
        if (picasso.loggingEnabled) {
          Utils.log(Utils.OWNER_MAIN, Utils.VERB_COMPLETED, request.plainId(), "from " + Picasso.LoadedFrom.MEMORY);
        }
        if (callback != null) {
