...
public class ImageLoader {
     ...
     * 我们可以合并多个请求到同一个url中去，使用一个request去请求数据
     */
    private final HashMap<String, BatchedImageRequest> mInFlightRequests =
            new HashMap<String, BatchedImageRequest>();

    /** HashMap of the currently pending responses (waiting to be delivered).
     * 正在等待的responses，等待被投递出去*/
    ...
    /** Runnable for in-flight response delivery.为in-flight的请求服务 */
    ...
    public class ImageContainer {
        ...
        public void cancelRequest() {
            ...
            //通过cachekey去获取request
            BatchedImageRequest request = mInFlightRequests.get(mCacheKey);
            //如果可以找到request
            if (request != null) {
                //在BatchedImageRequest中一个ImageContainer的列表里删去该container
                // 如果删完发现该列表的大小为0的话，就删掉该BatchedImageRequest
                boolean canceled = request.removeContainerAndCancelIfNecessary(this);
                if (canceled) {
                    mInFlightRequests.remove(mCacheKey);
                }
            } else {
                //如果该网络请求已经返回并且得到bitmap，现在在等待被显示出来的队列中的话，就将其删除
