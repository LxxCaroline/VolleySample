...
/*
 * 有了加载任务，具体的图片下载与解析是在哪里呢？这些都是耗时的操作，应该放在异步线程中进行，就是BitmapHunter。
 * BitmapHunter是一个Runnable，其中有一个decodeStream的抽象方法，用于子类实现不同类型资源的解析。
 */
class BitmapHunter implements Runnable {
    ...
    //为每个线程都提供一个不同的该变量，线程之间互不影响，为每个线程都提供给一个名字
    ...
    Bitmap result;
    Future<?> future;
    //记录该bitmap从哪里读取，分别有MEMORY,DISK,NETWORK
            ...
            if (data.needsTransformation() || exifOrientation != 0) {
                synchronized (DECODE_LOCK) {
                    ...
                    if (data.hasCustomTransformations()) {
                        ...
                        if (picasso.loggingEnabled) {
                            Utils.log(Utils.OWNER_HUNTER, Utils.VERB_TRANSFORMED, data.logId(), "from custom " +
                                    "transformations");
                        }
                    }
                }
        ...
        if (this.action == null) {
            ...
            if (loggingEnabled) {
                if (actions == null || actions.isEmpty()) {
                    ...
                    Utils.log(Utils.OWNER_HUNTER, Utils.VERB_JOINED, request.logId(), Utils.getLogIdsForHunter(this,
                            "to "));
                }
            }
            return;
    ...
    void detach(Action action) {
        ...
        if (picasso.loggingEnabled) {
            Utils.log(Utils.OWNER_HUNTER, Utils.VERB_REMOVED, action.request.logId(), Utils.getLogIdsForHunter(this,
                    "from "));
        }
    }
